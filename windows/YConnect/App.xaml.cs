...
namespace YConnect
{
    public partial class App : Application
    {
        ...
        protected override async void OnStartup(StartupEventArgs e)
        {
            ...
            try
            {
                var smoke = e.Args.Contains("--smoke"); var verifyRecharge = e.Args.Contains("--verify-recharge"); var verifyLogin = e.Args.Contains("--verify-login"); var demo = smoke || verifyRecharge || e.Args.Contains("--demo"); var development = demo || verifyLogin || e.Args.Contains("--development");
                var output = e.Args.FirstOrDefault(a => a.StartsWith("--output="))?.Substring(9);
                if ((smoke || verifyLogin || verifyRecharge) && string.IsNullOrEmpty(output)) throw new InvalidOperationException("Validation requires --output=<directory>");
                var root = smoke || verifyLogin || verifyRecharge ? Path.Combine(Path.GetFullPath(output), "sandbox") : null;
                var identity = smoke || verifyLogin || verifyRecharge ? "Validation-" + Guid.NewGuid().ToString("N") : demo ? "Demo" : development ? "Dev" : "Production";
                ...
                if (e.Args.Contains("--recharge") && !development) Controller.OpenRecharge();
                ...
                else if (verifyRecharge) { var successful = await Validation.SmokeRunner.RunRecharge(Controller, Path.GetFullPath(output)); Controller.Quit(); Environment.ExitCode = successful ? 0 : 1; }
