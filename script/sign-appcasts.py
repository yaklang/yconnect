...
def generate(directory, private_key, public_key):
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        for platform, kind, name in [('darwin', 'dmg', 'appcast-macos.xml'), ('windows', 'setup', 'appcast-windows.xml')]:
            asset, = [a for a in manifest['assets'] if a['platform'] == platform and a['kind'] == kind]
            ...
            ET.SubElement(channel, 'title').text = 'Y CONNECT updates'
            ET.SubElement(channel, 'language').text = 'zh-CN'
            item = ET.SubElement(channel, 'item')
            ET.SubElement(item, 'title').text = f"Y CONNECT {manifest['version']}"
            ...
            for field, value in [('version', build), ('shortVersionString', manifest['version']), ('edSignature', base64.b64encode(signature).decode()), ('os', 'macos' if platform == 'darwin' else 'windows-x64')]:
                enclosure.set(f'{{{NS}}}{field}', value)
            if platform == 'windows':
                enclosure.set(f'{{{NS}}}installerArguments', '/SILENT /SP- /NORESTART /NOFORCECLOSEAPPLICATIONS /YCONNECTUPDATE=1')
            ET.indent(rss)
...
if __name__ == '__main__':
    generate(sys.argv[1], os.environ['YCONNECT_UPDATE_PRIVATE_KEY'], (ROOT / 'resources/updates/ed25519-public-key.txt').read_text())
