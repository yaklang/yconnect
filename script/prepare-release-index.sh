...
jq -e --arg version "$VERSION" --arg base "$PUBLIC_BASE_URL/$VERSION/" '
  .schema_version == 1 and .product == "yconnect" and .version == $version and
  (.assets | type == "array" and length == 3) and
  ([.assets[] | .platform + ":" + .architecture + ":" + .kind] | sort) ==
    ["darwin:universal:dmg", "windows:amd64:setup", "windows:amd64:zip"] and
  all(.assets[];
    (.platform == "darwin" or .platform == "windows") and
    (.architecture == "universal" or .architecture == "amd64") and
    (.filename | type == "string" and test("^[^/\\\\]+$")) and
    (.url == ($base + .filename)) and
...
if [[ -n "${EXISTING_RELEASES_FILE:-}" ]]; then
  ...
  case "$status" in
    200) ;;
    404) printf '%s\n' '{"schema_version":1,"product":"yconnect","latest":"","versions":[]}' > "$OUT_DIR/releases.previous.json" ;;
    *) die "unexpected HTTP $status while fetching release index" ;;
  esac
fi

jq -e '.schema_version == 1 and .product == "yconnect" and (.versions | type == "array")' \
  "$OUT_DIR/releases.previous.json" >/dev/null || die "existing releases index is invalid"

python3 - "$VERSION" "$OUT_DIR/releases.previous.json" <<'CHECK_VERSION'
import json, sys
version = tuple(map(int, sys.argv[1].split('.')))
previous = json.load(open(sys.argv[2])).get('latest', '')
if previous and tuple(map(int, previous.split('.'))) > version:
...
echo "Prepared Y CONNECT release index for $VERSION"
